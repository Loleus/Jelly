    ...
    <div className="pointer-events-none fixed inset-0 z-40 animate-in fade-in duration-500">
      ...
      <div className="menu-title pointer-events-none absolute inset-x-4 top-[8%] text-center [@media(max-height:500px)]:top-[3.0rem]">
        <h1 className="font-freckle leading-[0.85] tracking-wide">
          <span className="block drop-shadow-[0px_0px_8px_rgba(86,250,200,0.9)] text-transparent text-[6rem] bg-clip-text bg-gradient-to-b from-[#a8c8ea] via-[#7fe6b1] to-[#4ade80] sm:text-[11rem] [@media(max-height:500px)]:text-[6rem] [@media(max-width:500px)]:mt-1">
            {t.appName}
          </span>
      ...
      <div className="pointer-events-auto absolute left-1/2 top-[62%] flex -translate-x-1/2 -translate-y-1/2 flex-col items-center [@media(max-height:500px)]:top-[68%]">
        <button
          onClick={onPlay}
          className="group relative flex h-15 w-15 items-center justify-center rounded-full bg-amber-400 text-[#a27c18] transition-transform duration-300 hover:scale-110 active:scale-95 sm:h-24 sm:w-24 [@media(max-height:500px)]:h-12 [@media(max-height:500px)]:w-12"
        ...
        <span className="mt-6 font-freckle text-2xl tracking-[0.3em] text-amber-400 drop-shadow-[0_3px_12px_rgba(0,0,0,0.9)] [@media(max-height:500px)]:mt-2 [@media(max-height:500px)]:text-base">
          {t.menuPlay}
        </span>
      </div>
